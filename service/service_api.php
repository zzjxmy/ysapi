...
class server{
    ...
    public function onConnect(swoole_server $serv, $fd, $from_id){
        $this->ids[$fd]=uniqid(); // 这里看情况上否要用个定时器做清理
...
//        $router = $this->yaf->getDispatcher()->getRouter();
//        $route = new \Yaf\Route\Simple($module, $controller, $action);
//        $router->addRoute("name", $route);
//        $request = new \Yaf\Request\Simple('CLI', $module, $controller, $action, $data);
//        $request->task_id=$taskId;
        $request = new \Yaf\Request\Http($module . '/' .$controller . '/' . $action);
